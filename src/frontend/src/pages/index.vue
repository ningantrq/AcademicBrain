...
const userInfo = ref<any>(null);  // 默认 null，表示未登录
...
const close = (val: boolean) => {
  loginShow.value = val;
  userInfo.value = userStore.fetchUserInfo();
};
// const close = (val: boolean) => {
//   loginShow.value = val;
//   // 切换登录框状态后，再次尝试获取用户信息
...
const getChatUserListMethod = () => {
  return new Promise<number>((resolve) => {
    getChatUserList().then((res: any) => {
      ...
      data.forEach((item: any) => {
        totalChatCount += item.count;
      });
      _countMessage.chatCount = totalChatCount;
      imStore.setCountMessage(_countMessage);
      imStore.setUserList(data);
      resolve(totalChatCount);
    }).catch(() => {
      resolve(0);
    });
  });
...
const getCountMessageMethod = () => {
  return new Promise<any>((resolve) => {
    getCountMessage().then((res: any) => {
      ...
      resolve({ likeOrCollectionCount: 0, commentCount: 0, followCount: 0 });
    });
  });
};
const login = () => {
...
const initData = async () => {
  ...
  // 如果已经登录，去获取一次聊天列表 & 消息统计（不使用 WebSocket）
  if (userInfo.value && userInfo.value.id) {
    await getChatUserListMethod();
    await getCountMessageMethod();
  }
};
initData();
</script>
